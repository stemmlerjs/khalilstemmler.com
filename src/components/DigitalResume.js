        ...
        elements: [
          {
            ...
            header: 'Brock University, Computer Science Honors (1st Class Standing)',
...
const SelectionNav = ({ currentPage, handleChangePage }) => {
  return (
    <div className={styles.selectionNavContainer}>
      ...
      <div onClick={() => handleChangePage('DEVELOPMENT')} 
        className={currentPage == "DEVELOPMENT" 
          ? `${styles.selectionNavItem} ${styles.selected}` 
          : styles.selectionNavItem}>Development
      </div>

      <div onClick={() => handleChangePage('EXPERIENCE')} 
        className={currentPage == "EXPERIENCE"
         ? `${styles.selectionNavItem} ${styles.selected}` 
         : styles.selectionNavItem}>Experience</div>
      <div 
        onClick={() => handleChangePage('EDUCATION')} 
        className={currentPage == "EDUCATION"
         ? `${styles.selectionNavItem} ${styles.selected}` 
         : styles.selectionNavItem}>Education</div>
    </div>
  )
}
  ...
  render () {
    return (
      ...
      <div className={styles.heroContainer}>
        <div className={styles.imageContainer}><img src={emojiis}/></div>
        <div className={styles.heroTextContainer}>
          <h3>For the love of building</h3>
          <div>Making things on the web is my passion. Here's my virtual resume.</div>
        </div>
      </div>
